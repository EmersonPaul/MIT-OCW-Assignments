...
def match_with_gaps(my_word, other_word):
    ...
    word_letters = my_word.replace(' ','') # Remove spaces from my_word
    if len(word_letters) == len(other_word):
        # This loop is used for comparing if each LETTER in the same position of words are the same
        for i in range(len(word_letters)):
            if word_letters[i] in alphabet and word_letters[i] == other_word[i]:
                matched_letters += 1
            elif word_letters[i] in alphabet and word_letters[i] != other_word[i]:
                return False
        # Compares if '_'s appear more than once by subtracting matched_letters to length of other_word
        if abs(matched_letters - len(other_word)) > 1: 
            return True
        else:
            return False                                                     
    else:
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while guesses > 0:
        ...
        if letter_guess in secret_word and letter_guess not in letters_guessed:
        ...
        elif letter_guess not in string.ascii_lowercase and letter_guess != '*':
            warning -= 1
            print('Oops! That is not a valid letter! You have', warning, 'warnings left.',
                  guessed_word)
        elif letter_guess in letters_guessed:
            ...
            if abs(guessed_word_length - guessed_word_character) < guessed_word_length - 1:
                ...
                print("Hints unavailable. Keep guessing!")        
        else:
            if letter_guess not in vowels and letter_guess not in secret_word:
                guesses -= 1
            elif letter_guess in vowels and letter_guess not in secret_word:
